...
class ndbcSpectra(object):
    def __init__(self, buoy='46232',datasource='http',e=[], units='ft', **kwargs):
        self.buoy = buoy
        if datasource == 'local':
    ...
    def jsonify(self, dataType='spectra'):
        js = {'timestamp': self.timestamp.isoformat(' '), 'disclaimer': "Data in this object has not been validated and should be considered a placeholder"} # 'buoy': self.buoy,
        jsList = []
        digits = 3
        if dataType is 'spectra':
            ...
            for i in b:
                ...
                jsList.append(dip)
        elif dataType is 'nineBand':
            b = self.nineBand()
            keys = ['22','20','17','15','13','11','9','7','4']
            jsList = [{'band':k,'height':v,'direction':v2} for k,v,v2 in zip(keys,b[0],b[1])]
    ...
    def nineBand(self):
        ...
        while fence < 9:
            ...
            fence +=1
        self.nineHeights = [round(2*4*self.units*.01*sqrt(float(v[0])), 2) for v in self.nineEnergy]
...
def main(buoy='46232', datatype='nineBand', units=3.28, json=False, datasource='http', **kwargs):
    if __name__ == "__main__":
